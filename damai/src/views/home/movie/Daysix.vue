<template>
  <div class="container" ref="daylist">
    <ul>
      <li v-for="item in daylist.currentCity" :key="item.currentTime">
...
export default {
  ...
  //     // 100px / 3.75 = 26.667vw
  //     return 26.667 * this.daylist.currentCity.length + "vw";
  //   },
  // },
  created() {
    this.getExpectList();
  ...
  methods: {
    async getExpectList() {
      let rs = await get("/day6.json");
      console.log(rs);
      this.daylist = rs.data.data;
      // console.log(rs.data.data.currentCity.length);
      // 判断数据是否全部完毕
      if (rs.data.data.currentCity.length < this.daylist.limit) {
